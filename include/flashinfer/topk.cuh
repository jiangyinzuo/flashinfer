...
inline bool ShouldUseFilteredTopK(uint32_t num_rows, uint32_t top_k_val, uint32_t max_len,
  ...
  // Deterministic mode: measured on bf16/fp16/fp32 with seq in [16K, 128K],
  // k in [512, 2048], and rows in [1, 128], radix multi-CTA consistently
  // outperforms filtered for seq >= 32K, and also wins at seq=16K for large
  // row counts. Keep a compact rule set based on these boundaries.
  if (deterministic) {
    using OrderedType = typename RadixTopKTraits<DType>::OrderedType;
    constexpr uint32_t BLOCK_THREADS = 1024;
    const uint32_t vec_size = std::gcd(16 / sizeof(DType), max_len);
    ...
    if (cudaGetDevice(&device) != cudaSuccess ||
        cudaDeviceGetAttribute(&max_smem_per_block, cudaDevAttrMaxSharedMemoryPerBlockOptin,
                               device) != cudaSuccess) {
      // Conservative fallback when device query fails.
      return true;
    }
    ...
    const bool radix_single_cta = (max_len <= max_chunk_elements);
    if (radix_single_cta) {
      return false;
    }
    ...
    // At the 16K boundary, large batches also favor radix multi-CTA.
    if (max_len >= 16384 && num_rows >= 64) {
      return false;
    }
