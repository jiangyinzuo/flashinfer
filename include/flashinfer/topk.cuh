...
using RadixTopKDetBlockScanT = cub::BlockScan<uint32_t, 1024, cub::BLOCK_SCAN_RAKING_MEMOIZE>;
...
struct RadixDeterministicCollectScratch {
  uint32_t gt_count[RADIX_TOPK_MAX_CTAS_PER_GROUP];
  uint32_t eq_count[RADIX_TOPK_MAX_CTAS_PER_GROUP];
};

inline RadixDeterministicCollectScratch* GetRadixDeterministicCollectScratchBuffer(
    RadixRowState* row_states_buffer, uint32_t num_groups) {
  return reinterpret_cast<RadixDeterministicCollectScratch*>(row_states_buffer + num_groups);
}
...
template <uint32_t BLOCK_THREADS>
__device__ __forceinline__ void CleanupRadixRowStateAtKernelTail(RadixRowState* state,
  ...
  for (uint32_t buf = 0; buf < 3; ++buf) {
    for (uint32_t i = tx; i < RADIX_TOPK_RADIX; i += BLOCK_THREADS) {
...
inline size_t GetRadixRowStateWorkspaceBytes(uint32_t num_groups, bool include_det_scratch) {
  size_t bytes = static_cast<size_t>(num_groups) * sizeof(RadixRowState);
  if (include_det_scratch) {
    bytes += static_cast<size_t>(num_groups) * sizeof(RadixDeterministicCollectScratch);
  }
  return bytes;
}

inline cudaError_t CleanupRadixRowStateForNextLaunch(RadixRowState* row_states, uint32_t num_groups,
                                                     bool include_det_scratch,
  ...
  const size_t bytes = GetRadixRowStateWorkspaceBytes(num_groups, include_det_scratch);
  return cudaMemsetAsync(row_states, 0, bytes, stream);
}
...
__device__ __forceinline__ void RadixSelectOneRound(
  ...
  if constexpr (!SINGLE_CTA) {
    ...
    AdvanceRadixGroupBarrier<BLOCK_THREADS>(state, barrier_phase, ctas_per_group, tx);
...
__device__ __forceinline__ void RadixCollectIndicesDeterministic(
    ...
    uint32_t* local_histogram, uint32_t* suffix_sum, RadixRowState* state,
    RadixDeterministicCollectScratch* det_scratch, int& barrier_phase, uint32_t ctas_per_group,
    uint32_t cta_in_group, uint32_t tx, OutputFunc output_func) {
...
#define no_eq_truncation suffix_sum[3]
  ...
  if constexpr (SINGLE_CTA) {
    ...
    if (tx == 0) {
      ...
      local_eq_take = 0;
      no_eq_truncation = 0;
      det_scratch->gt_count[cta_in_group] = local_gt_count;
      det_scratch->eq_count[cta_in_group] = local_eq_count_hint;
    }
    __syncthreads();
  ...
  if constexpr (SINGLE_CTA) {
    if (tx == 0) {
      total_gt = gt_total;
      eq_needed = (k > gt_total) ? (k - gt_total) : 0;
    }
  }
  __syncthreads();
  ...
  if (no_eq_truncation != 0) {
    // local_eq_count_hint is computed exactly by RadixSelectFromSharedMemory when
    // TRACK_EQ_COUNT=true, and eq_sum is the exact sum of per-CTA eq counts. no_eq_truncation is
    // only true when the full ==pivot set will be included, so this path can emit without checking
    // pos<k.
...
__device__ __forceinline__ void RadixCollectIndicesDispatch(
  ...
  if constexpr (DETERMINISTIC) {
    RadixCollectIndicesDeterministic<BLOCK_THREADS, SINGLE_CTA, OrderedType>(
        ...
        local_eq_count, local_histogram, suffix_sum, state, det_scratch, barrier_phase,
        ctas_per_group, cta_in_group, tx, output_func);
  } else {
...
__global__ void __launch_bounds__(BLOCK_THREADS) RadixTopKKernel_Unified(
    ...
    RadixDeterministicCollectScratch* det_scratch_states, uint32_t chunk_size,
...
#define shared_output_counter shared_scalars[4]
  ...
  if constexpr (!SINGLE_CTA && DETERMINISTIC) {
    det_scratch = &det_scratch_states[group_id];
  }
  ...
  for (uint32_t iter = 0; iter < total_iterations; iter++) {
    ...
    if constexpr (MODE == RadixTopKMode::Basic) {
      if (k >= length) {
        ...
        if constexpr (!SINGLE_CTA) {
          ...
          // Keep the next iteration from reusing this histogram buffer before CTA 0 finishes
          // clearing it for the first radix round of the following row.
          AdvanceRadixGroupBarrier<BLOCK_THREADS>(state, barrier_phase, ctas_per_group, tx);
        }
        continue;
      }
    ...
    if constexpr (MODE == RadixTopKMode::Basic) {
      ...
      RadixCollectIndicesDispatch<BLOCK_THREADS, SINGLE_CTA, DETERMINISTIC, OrderedType>(
          shared_ordered, actual_chunk_size, chunk_start, k, ordered_pivot, local_gt_count,
          local_eq_count, local_histogram, suffix_sum, &shared_output_counter, state, det_scratch,
          barrier_phase, ctas_per_group, cta_in_group, tx,
    ...
    if constexpr (!SINGLE_CTA) {
      // Keep iteration boundaries in lock-step. Without this rendezvous, one CTA can start the next
      // row and reset output_counter while a peer CTA is still issuing the final ==pivot atomicAdd
      // for the current row.
      AdvanceRadixGroupBarrier<BLOCK_THREADS>(state, barrier_phase, ctas_per_group, tx);
    }
  }
...
#undef shared_output_counter
  ...
  if constexpr (!SINGLE_CTA) {
    CleanupRadixRowStateAtKernelTail<BLOCK_THREADS>(state, cta_in_group, tx);
  }
}
...
cudaError_t RadixTopKMaskLogitsMultiCTA(DType* logits, DType* masked_logits, IdType* top_k_arr,
  ...
  if (!single_cta) {
    FLASHINFER_CUDA_CALL(
        CleanupRadixRowStateForNextLaunch(row_states_buffer, num_groups, false, stream));
  }
...
cudaError_t RadixTopKPageTableTransformMultiCTA(DType* input, IdType* output_page_table,
  ...
  RadixDeterministicCollectScratch* det_scratch_buffer =
      (!single_cta && deterministic && row_states_buffer != nullptr)
          ? GetRadixDeterministicCollectScratchBuffer(row_states_buffer, config.num_groups)
          : nullptr;
...
#pragma unroll 8
  ...
  auto build_det_pivot = [&](int stop_round) {
    if constexpr (sizeof(OrderedType) == 2) {
      uint32_t low_byte = (stop_round >= 0) ? static_cast<uint32_t>(s_refine_thresholds[0]) : 0xFFu;
      return static_cast<OrderedType>((static_cast<uint32_t>(threshold_bin) << 8) | low_byte);
    } else {
      uint32_t pivot = 0;
  ...
  auto collect_det_eq_for_pivot = [&](OrderedType pivot, int eq_needed) {
    if (eq_needed > 0) {
...
#pragma unroll 1
      for (int it = 0; it < num_iters; ++it) {
        ...
        bool matched = valid && (Traits::ToOrdered(score[idx]) == pivot);

        uint32_t ballot = __ballot_sync(0xFFFFFFFF, matched);
    ...
    if constexpr (NUM_ROUNDS == 1) {
      if (s_refine_overflow) {
        ...
        if constexpr (DETERMINISTIC) {
          ...
          collect_det_eq_for_pivot(static_cast<OrderedType>((static_cast<int>(threshold_bin) << 8) |
        ...
        if constexpr (DETERMINISTIC) {
          collect_det_eq_for_pivot(build_det_pivot(/*stop_round=*/0), topk);
        }
      }
    } else {
...
#pragma unroll
      ...
      if constexpr (DETERMINISTIC) {
        if (!s_refine_overflow) {
          collect_det_eq_for_pivot(build_det_pivot(det_stop_round), topk);
        }
      }
...
#pragma unroll
        ...
        __syncthreads();
        if constexpr (DETERMINISTIC) {
          collect_det_eq_for_pivot(static_cast<OrderedType>(pivot), eq_needed);
        }
      }
    }
  }
...
struct SortTopKByIndexKernelTraits;

template <uint32_t BLOCK_THREADS, uint32_t ITEMS_PER_THREAD, typename DType>
struct SortTopKByIndexKernelTraits<FilteredTopKMode::Plain, BLOCK_THREADS, ITEMS_PER_THREAD,
                                   DType> {
  using BlockRadixSortT = cub::BlockRadixSort<uint32_t, BLOCK_THREADS, ITEMS_PER_THREAD, DType>;
  using TempStorage = typename BlockRadixSortT::TempStorage;
};

template <uint32_t BLOCK_THREADS, uint32_t ITEMS_PER_THREAD, typename DType>
struct SortTopKByIndexKernelTraits<FilteredTopKMode::PageTable, BLOCK_THREADS, ITEMS_PER_THREAD,
                                   DType> {
  using BlockRadixSortT = cub::BlockRadixSort<uint32_t, BLOCK_THREADS, ITEMS_PER_THREAD>;
  using TempStorage = typename BlockRadixSortT::TempStorage;
};

template <uint32_t BLOCK_THREADS, uint32_t ITEMS_PER_THREAD, typename DType>
struct SortTopKByIndexKernelTraits<FilteredTopKMode::Ragged, BLOCK_THREADS, ITEMS_PER_THREAD,
                                   DType> {
  using BlockRadixSortT = cub::BlockRadixSort<uint32_t, BLOCK_THREADS, ITEMS_PER_THREAD>;
  using TempStorage = typename BlockRadixSortT::TempStorage;
};
...
__global__ void __launch_bounds__(BLOCK_THREADS)
    SortTopKByIndexKernel(IdType* output_indices, DType* output_values, const IdType* aux_input,
                          ...
                          uint32_t max_len) {
  using KernelTraits = SortTopKByIndexKernelTraits<MODE, BLOCK_THREADS, ITEMS_PER_THREAD, DType>;
  using BlockRadixSortT = typename KernelTraits::BlockRadixSortT;
  __shared__ typename KernelTraits::TempStorage temp_storage;
...
template <typename DType, typename IdType>
cudaError_t SortTopKByIndex(IdType* output_indices, DType* output_values, uint32_t num_rows,
                            uint32_t top_k_val, uint32_t max_len, cudaStream_t stream = 0) {
  return LaunchSortTopKByIndex<FilteredTopKMode::Plain, DType, IdType>(
      output_indices, output_values, nullptr, 0, nullptr, num_rows, top_k_val, max_len, stream);
}

template <typename IdType>
cudaError_t SortTopKPageTableByIndex(IdType* output_page_table, const IdType* src_page_table,
                                     int64_t src_stride, const IdType* row_to_batch,
                                     uint32_t num_rows, uint32_t top_k_val, uint32_t max_len,
                                     cudaStream_t stream = 0) {
  return LaunchSortTopKByIndex<FilteredTopKMode::PageTable, uint8_t, IdType>(
      output_page_table, static_cast<uint8_t*>(nullptr), src_page_table, src_stride, row_to_batch,
      num_rows, top_k_val, max_len, stream);
}

template <typename IdType>
cudaError_t SortTopKRaggedByIndex(IdType* output_indices, const IdType* offsets, uint32_t num_rows,
                                  uint32_t top_k_val, uint32_t max_len, cudaStream_t stream = 0) {
  return LaunchSortTopKByIndex<FilteredTopKMode::Ragged, uint8_t, IdType>(
...
inline bool ShouldUseFilteredTopK(uint32_t num_rows, uint32_t top_k_val, uint32_t max_len,
  ...
  if (deterministic) {
    if constexpr (sizeof(DType) <= 2) {
      ...
      // Prefer FilteredTopK for short and medium contexts, but switch to radix
      // once fp32 deterministic workloads become long enough that deterministic
      // tie cleanup dominates the filtered path.
      const bool prefer_radix_decode = (top_k_val >= 2048) && (max_len >= 65536) && (num_rows <= 8);
      const bool prefer_radix_long_context =
          ((max_len >= 131072) && (num_rows <= 32)) || ((max_len >= 65536) && (num_rows == 1));
      if (prefer_radix_decode || prefer_radix_long_context) {
        return false;
      }
      return true;
    }
...
cudaError_t TopKPageTableTransformDispatch(DType* input, IdType* output_page_table,
  ...
  auto run_filtered = [&]() -> cudaError_t {
    cudaError_t status = FilteredTopKPageTableTransform<DType, IdType>(
    ...
    if (deterministic) {
      cudaError_t sort_status =
          SortTopKPageTableByIndex<IdType>(output_page_table, src_page_table, src_stride,
...
cudaError_t TopKRaggedTransformDispatch(DType* input, IdType* output_indices, const IdType* offsets,
  ...
  const bool can_implement_filtered = CanImplementFilteredTopK();
  auto run_filtered = [&]() -> cudaError_t {
    cudaError_t status = FilteredTopKRaggedTransform<DType, IdType>(input, output_indices, offsets,
                                                                    lengths, num_rows, top_k_val,
                                                                    max_len, deterministic, stream);
    if (status != cudaSuccess) {
      return status;
    }
    if (deterministic) {
      cudaError_t sort_status = SortTopKRaggedByIndex<IdType>(output_indices, offsets, num_rows,
