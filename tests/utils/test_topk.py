...
@pytest.mark.parametrize("algo", ["auto", "multi_cta", "filtered"])
def test_top_k_deterministic_sorted_large_k_matches_torch_by_algo(algo, set_topk_algo):
    """Large-k deterministic sorted output should match torch.topk across routing modes."""
    set_topk_algo(algo)
...
def test_top_k_deterministic_trivial_k_equals_length_by_algo(algo, set_topk_algo):
    """Deterministic large-k fast paths should remain exact across routing modes."""
    set_topk_algo(algo)
